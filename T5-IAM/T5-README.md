...
I also tried to attach a role to the Instance using terraform, but it didn't work.     
...
## Here's basically how it went down in the end
- **Step 1:** Stop the VM
...
- **Step 3:** Spin it back up and download awscli: `sudo apt install -y awscli`
...
# Side note unrelated
The `main.tf` file was getting a bit heavy, so I refactored it into a modular monolithic structure.      
Each `*.tf` file now has a distinct purpose, which also applies to the main `terraform` folder!
